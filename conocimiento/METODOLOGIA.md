# METODOLOGIA

**Proyecto:** Voces de las Nubes  
**Versión:** 1.0  
**Estado:** Borrador consolidado  
**Fecha:** 2026-08-05  

---

# 1. Propósito metodológico

Este documento responde a una sola pregunta:

> ¿Cómo trabaja Voces de las Nubes?

Voces de las Nubes construye materiales para el aprendizaje del Didxazá mediante un proceso colaborativo que articula observación de situaciones comunicativas reales, diseño pedagógico, trabajo directo con hablantes, revisión bibliográfica, producción oral, documentación de decisiones y validación especializada.

La metodología no parte de una lista cerrada de contenidos lingüísticos ni de una traducción lineal desde el español. Parte de usos comunicativos situados y distribuye la autoridad entre participantes según el tipo de decisión que deba tomarse.

Este documento define el método general del proyecto. No describe en detalle la arquitectura del corpus, la producción técnica del audio, la fundamentación teórica, la bibliografía ni la administración del repositorio.

---

# 2. Filosofía del proyecto

## 2.1 La lengua se trabaja como práctica viva

El Didxazá no se trata únicamente como un sistema gramatical documentado, sino como una práctica oral, cultural y situada.

Las formas lingüísticas deben relacionarse con:

- situaciones reales de interacción;
- funciones comunicativas concretas;
- usos reconocibles por los hablantes;
- variación lingüística y cultural;
- condiciones actuales de transmisión y aprendizaje.

La bibliografía orienta y permite anticipar problemas, pero no sustituye la observación de campo ni la validación comunitaria.

## 2.2 El audio tiene prioridad cuando la escritura es incierta

Cuando una transcripción, una representación ortográfica o una interpretación posterior entra en conflicto con una producción oral registrada, el audio validado constituye la referencia principal.

La escritura sigue siendo necesaria para organizar, buscar, enseñar y documentar el material, pero no debe presentarse como una representación indiscutible cuando el hablante no la ha confirmado con seguridad.

## 2.3 El aprendizaje se construye desde la escucha hacia la producción

El punto de entrada principal es la comprensión auditiva progresiva.

Los materiales deben permitir que la persona:

1. escuche una expresión contextualizada;
2. intente anticiparla o producirla;
3. compare su intento con un modelo;
4. vuelva a escucharla;
5. reutilice la estructura en nuevos contextos.

La gramática se utiliza como apoyo para comprender y controlar el sistema, no como único punto de partida ni como orden automático de enseñanza.

## 2.4 La participación de los hablantes es intelectual, no instrumental

Los hablantes no participan solo como traductores, informantes o voces de grabación.

Intervienen como autoridades en:

- naturalidad;
- significado;
- pronunciación;
- uso;
- pertinencia cultural;
- variación;
- escritura;
- selección y reformulación de contenidos.

Su participación debe reconocerse en la autoría, los créditos y las decisiones que efectivamente hayan validado.

## 2.5 La metodología permanece abierta a corrección

Las decisiones vigentes orientan el trabajo, pero pueden revisarse cuando:

- la evidencia del proyecto las contradiga;
- una prueba produzca resultados deficientes;
- los hablantes identifiquen artificialidad o inadecuación;
- aparezca una fuente especializada que obligue a reconsiderarlas;
- cambien las condiciones reales de producción o aprendizaje.

Las contradicciones no se eliminan para mantener una falsa coherencia. Se documentan y se resuelven según el ámbito de autoridad correspondiente.

---

# 3. Principios metodológicos

## 3.1 Situaciones antes que listas temáticas

El trabajo parte de escenas y necesidades comunicativas de la vida cotidiana.

Las categorías temáticas pueden utilizarse para revisar cobertura, pero no deben gobernar por sí solas la generación del material.

Una situación comunicativa no se convierte automáticamente en una lección. Primero debe analizarse para identificar qué acciones lingüísticas contiene y qué recursos exige.

## 3.2 Funciones comunicativas antes que acumulación de frases

La unidad funcional del aprendizaje es la acción que una persona logra realizar con la lengua.

Ejemplos:

- saludar;
- presentarse;
- pedir ayuda;
- localizar algo;
- expresar necesidad;
- confirmar;
- rechazar;
- agradecer;
- despedirse.

La cantidad de frases no constituye por sí misma una medida suficiente de avance. Varias frases pueden enseñar la misma función sin ampliar realmente la capacidad comunicativa.

## 3.3 Producción oral integrada desde el diseño

Cada enunciado debe concebirse como material que el aprendiz podrá intentar producir, no solo reconocer.

Las pausas, repeticiones y secuencias de escucha deben crear oportunidades de recuperación activa y comparación con el modelo.

La producción no se añade al final del proceso. Se considera desde la selección inicial del material.

## 3.4 Reutilización productiva

Las estructuras ya introducidas deben reaparecer en contextos diferentes.

La progresión se construye mediante:

- reutilización de patrones conocidos;
- introducción controlada de novedades;
- variación contextual;
- aumento gradual de complejidad;
- reducción de repeticiones funcionalmente vacías.

Repetir una frase con un dato distinto no constituye necesariamente aprendizaje nuevo.

## 3.5 Naturalidad antes que regularidad artificial

El material debe conservar la irregularidad razonable del habla humana.

Se evita:

- alternancia mecánica de pregunta y respuesta;
- saludos y despedidas obligatorios en toda escena;
- diálogos construidos para cumplir listas;
- líneas añadidas como relleno;
- traducción demasiado literal;
- progresiones que produzcan habla robótica.

La naturalidad no elimina la planificación pedagógica. Obliga a que esa planificación no sea visible como artificio.

## 3.6 Economía del habla

Las escenas deben desarrollar únicamente lo necesario para cumplir su propósito comunicativo.

La longitud no se alcanza mediante turnos artificialmente extensos, explicaciones impropias del contexto o repeticiones decorativas.

Cuando se requieren conversaciones largas, deben crecer mediante más turnos naturales, reacciones, reformulaciones y pequeñas variaciones de la situación.

## 3.7 Control externo de cobertura cuando sea necesario

El modelo que genera borradores no debe asumir tareas estadísticas que no puede sostener con fiabilidad entre sesiones.

El historial de:

- situaciones;
- patrones;
- léxico;
- personajes;
- balance;
- cobertura acumulada;

debe mantenerse mediante herramientas o registros externos cuando el volumen del corpus lo exija.

Las decisiones ya resueltas por ese control externo deben entregarse al proceso de generación como instrucciones concretas, no como datos crudos que el modelo deba interpretar nuevamente.

## 3.8 Separación entre evidencia, interpretación y decisión

El proyecto distingue:

- lo observado o registrado;
- lo interpretado;
- lo supuesto;
- lo decidido;
- lo validado;
- lo pendiente.

Una propuesta producida por una herramienta automática no constituye evidencia. Solo entra en vigor cuando el proyecto la adopta explícitamente mediante revisión.

---

# 4. Flujo general de trabajo

## 4.1 Identificación de una necesidad comunicativa

El proceso comienza con una situación, interacción o necesidad real que resulte pertinente para el uso cotidiano del Didxazá.

La situación puede proceder de:

- observación;
- experiencia comunitaria;
- propuesta de un hablante;
- revisión del material existente;
- detección de una carencia del corpus;
- requerimiento pedagógico.

## 4.2 Descomposición en funciones

La situación se analiza para identificar:

- qué intenta hacer cada participante;
- qué funciones comunicativas aparecen;
- qué turnos son indispensables;
- qué recursos ya existen;
- qué elementos serían nuevos;
- qué parte requiere validación cultural.

## 4.3 Revisión de cobertura previa

Antes de crear material nuevo se revisa lo ya construido para detectar:

- repeticiones literales;
- funciones ya cubiertas;
- patrones ya introducidos;
- variantes sin ganancia pedagógica;
- vocabulario ya suficientemente reforzado;
- vacíos reales.

La revisión evita que el proyecto confunda crecimiento numérico con avance pedagógico.

## 4.4 Preparación de microescenas

La necesidad se convierte en una microescena manejable.

La microescena especifica, como mínimo:

- contexto;
- participantes;
- objetivo de cada participante;
- funciones centrales;
- información necesaria;
- posibles restricciones culturales;
- elementos que deben ser elicitados y no inventados.

## 4.5 Elicitación y construcción con hablantes

La sesión con hablantes no se limita a pedir traducciones aisladas.

Se busca reconstruir:

- cómo ocurriría realmente la interacción;
- qué se diría;
- qué no se diría;
- qué variantes son posibles;
- qué expresiones resultan artificiales;
- cómo cambia la forma según contexto, edad, confianza o localidad;
- qué elementos requieren explicación adicional.

Las propuestas en español funcionan como apoyo o punto de partida, no como molde obligatorio de la estructura final en Didxazá.

## 4.6 Organización pedagógica

Después de obtener y validar el material lingüístico, se determina su lugar dentro del proceso de aprendizaje.

Se considera:

- complejidad;
- productividad;
- dependencia de estructuras previas;
- posibilidad de reutilización;
- longitud;
- facilidad de producción;
- pertinencia comunicativa;
- relación con materiales anteriores.

La frecuencia cotidiana orienta la selección, pero no determina por sí sola el orden.

## 4.7 Producción del material

El contenido aprobado pasa a las etapas de registro, edición, catalogación y preparación de formatos de aprendizaje.

Los procedimientos específicos de grabación, edición, niveles, exportación y publicación se documentan en `AUDIO.md`.

## 4.8 Revisión y control de calidad

Antes de considerarse listo, el material debe superar las revisiones que correspondan:

- lingüística;
- comunitaria;
- pedagógica;
- técnica;
- documental.

No todas las revisiones requieren a la misma persona. La autoridad depende del objeto revisado.

## 4.9 Registro de decisiones

Las decisiones relevantes deben conservar:

- formulación;
- justificación;
- fuente;
- responsable;
- validadores;
- alcance;
- estado;
- condiciones de revisión;
- relación con decisiones anteriores.

Las decisiones reemplazadas se conservan como historia, pero no se reutilizan como instrucciones vigentes.

## 4.10 Publicación y retroalimentación

La publicación no cierra definitivamente el material.

La observación de uso puede revelar:

- problemas de comprensión;
- pausas insuficientes;
- dificultad de producción;
- expresiones poco naturales;
- secuencias mal graduadas;
- problemas de acceso;
- errores de transcripción o catalogación.

La retroalimentación vuelve al proceso como evidencia para corregir versiones posteriores.

---

# 5. Diseño del proceso de aprendizaje

## 5.1 Progresión

La progresión se organiza mediante la combinación de:

- funciones comunicativas;
- patrones reutilizables;
- conocimiento previo;
- complejidad lingüística;
- carga pragmática;
- contexto;
- capacidad real de producción.

No se ordena automáticamente por capítulos gramaticales, temas léxicos ni cronología de actividades cotidianas.

## 5.2 Introducción de novedades

Una unidad debe introducir novedades controladas sobre una base mayoritariamente conocida.

La novedad puede consistir en:

- una función;
- una estructura;
- una relación pragmática;
- una forma de negación;
- un tipo de complementación;
- una expresión fija;
- una variante contextual.

No toda palabra nueva equivale a una dificultad estructural nueva.

## 5.3 Patrones

Un patrón se define como una estructura lingüística productiva, no como un verbo, un tema o una frase superficialmente semejante.

La metodología busca que un patrón aparezca en más de un contexto para que el aprendiz pueda reconocer su productividad.

## 5.4 Escucha reiterada

La repetición se utiliza para consolidar reconocimiento, anticipación y producción.

Debe evitar convertirse en repetición mecánica sin variación ni propósito.

La escucha reiterada es más útil cuando:

- el material es comprensible;
- existe un objetivo de atención;
- hay oportunidad de respuesta;
- la comparación con el modelo es inmediata;
- la estructura reaparece en otro contexto.

## 5.5 Pausas de producción

Las pausas son componentes pedagógicos del material.

Su función es permitir que la persona:

- recuerde;
- anticipe;
- produzca;
- detecte una carencia;
- compare su intento.

Su duración debe evaluarse con pruebas reales y ajustarse según la longitud y dificultad del enunciado.

## 5.6 Gramática como referencia

Las gramáticas descriptivas se utilizan para:

- anticipar diferencias relevantes con el español;
- identificar estructuras;
- revisar cobertura;
- detectar riesgos de simplificación;
- formular mejores preguntas para los hablantes;
- comprobar coherencia.

No se utilizan como método directo de enseñanza ni como sustituto de la lengua viva.

## 5.7 Evaluación

El avance debe evaluarse por lo que la persona puede comprender y realizar, no solo por el número de frases escuchadas o memorizadas.

La evaluación metodológica debe observar, cuando existan condiciones para hacerlo:

- reconocimiento auditivo;
- recuperación sin apoyo;
- producción;
- transferencia a otra situación;
- comprensión de variantes;
- retención;
- disposición para seguir usando el material.

El sistema de evaluación aún requiere desarrollo y prueba con usuarios.

---

# 6. Papel de cada participante

## 6.1 Coordinación y diseño metodológico

Emiliano López Carlton es responsable de:

- definir objetivos;
- organizar el proceso;
- identificar necesidades comunicativas;
- diseñar la progresión;
- preparar sesiones;
- mantener coherencia entre componentes;
- registrar decisiones;
- coordinar producción y publicación;
- distinguir qué requiere validación externa.

No le corresponde validar por sí mismo la naturalidad, pronunciación, gramática o consenso comunitario del Didxazá.

## 6.2 Hablantes y colaboradores comunitarios

Los hablantes participan en:

- elicitación;
- traducción;
- reformulación;
- producción oral;
- revisión;
- contextualización;
- identificación de variantes;
- decisiones ortográficas dentro de su alcance;
- validación de naturalidad y pertinencia.

Cuando intervienen sustancialmente en la construcción de transcripciones o materiales, deben reconocerse como coautores de ese trabajo.

La participación es voluntaria y debe basarse en acuerdos claros sobre uso, reconocimiento y alcance.

## 6.3 Especialistas

Los especialistas intervienen únicamente dentro de su ámbito.

Pueden revisar:

- metodología;
- pedagogía;
- lingüística;
- sociolingüística;
- audio;
- tecnología;
- documentación;
- ética;
- evaluación.

Su opinión no sustituye automáticamente la autoridad de los hablantes sobre el uso vivo de la lengua.

## 6.4 Institución

La Casa de las Ciencias de Oaxaca puede validar:

- coherencia institucional;
- objetivos;
- entregables;
- cronograma;
- condiciones administrativas;
- relación con sus programas.

La aprobación institucional no equivale a validación lingüística, comunitaria o pedagógica.

## 6.5 Herramientas automáticas

Las herramientas automáticas pueden apoyar:

- generación de borradores;
- comparación;
- clasificación;
- detección de duplicados;
- transformación de formatos;
- procesamiento por lotes;
- análisis preliminar.

No pueden:

- validar Didxazá;
- representar consenso comunitario;
- decidir naturalidad;
- convertir una hipótesis en evidencia;
- reemplazar la revisión humana pertinente.

---

# 7. Proceso de validación

## 7.1 Principio de autoridad pertinente

Cada objeto debe ser revisado por quien tenga autoridad sobre ese objeto.

Ejemplos:

- uso y naturalidad: hablantes;
- adecuación cultural: colaboradores comunitarios pertinentes;
- estructura lingüística: hablantes y, cuando corresponda, especialistas;
- progresión de aprendizaje: coordinación y revisión pedagógica;
- niveles y procesamiento de audio: pruebas técnicas y especialistas;
- compromisos institucionales: acuerdos formales;
- interpretación bibliográfica: lectura documentada y revisión especializada cuando sea necesaria.

## 7.2 Alcance explícito

Toda validación debe indicar:

- qué se revisó;
- quién lo revisó;
- con qué autoridad;
- qué resultado produjo;
- qué ajustes exigió;
- qué no quedó validado.

La aprobación de una parte no se extiende automáticamente al conjunto.

## 7.3 Desacuerdo entre hablantes

Cuando dos hablantes difieren, no se elige una forma de manera automática.

Se documenta:

- localidad;
- contexto;
- edad o perfil cuando sea pertinente;
- situación de uso;
- posible variación;
- grado de acuerdo;
- necesidad de nuevas consultas.

La diferencia puede representar variación legítima y no un error.

## 7.4 Bibliografía y validación comunitaria

La bibliografía puede:

- describir;
- fundamentar;
- ofrecer antecedentes;
- alertar sobre fenómenos;
- aportar categorías de análisis.

No reemplaza la validación del uso contemporáneo.

Cuando la bibliografía y la evidencia del proyecto difieren, la discrepancia debe documentarse y el alcance de cada una debe delimitarse.

## 7.5 Estado del material

El material debe distinguir, como mínimo, entre:

- borrador;
- en revisión;
- pendiente de validación;
- validado con alcance específico;
- aprobado para uso;
- reemplazado;
- descartado.

No debe publicarse como definitivo un contenido cuya validación necesaria siga pendiente.

---

# 8. Control de calidad

## 8.1 Calidad metodológica

Se verifica que:

- la situación sea pertinente;
- las funciones estén identificadas;
- exista ganancia pedagógica;
- la progresión dependa de conocimientos previos;
- no se duplique material sin propósito;
- la interacción no sea artificial;
- las responsabilidades estén correctamente distribuidas.

## 8.2 Calidad lingüística

Se verifica, según el alcance disponible:

- significado;
- naturalidad;
- pronunciación;
- segmentación;
- escritura;
- correspondencia entre audio y transcripción;
- contexto de uso;
- variación.

## 8.3 Calidad pedagógica

Se verifica que el material:

- sea comprensible para el nivel previsto;
- permita producción;
- reutilice estructuras;
- introduzca novedades controladas;
- evite sobrecarga;
- tenga pausas suficientes;
- pueda transferirse a situaciones distintas.

## 8.4 Calidad técnica

Se verifica que los archivos y formatos:

- sean reproducibles;
- conserven trazabilidad;
- mantengan calidad suficiente;
- no introduzcan alteraciones que afecten la percepción lingüística;
- puedan procesarse de forma sostenible;
- sean accesibles en los dispositivos previstos.

Los parámetros concretos se mantienen en `AUDIO.md`.

## 8.5 Calidad documental

Se verifica que:

- cada material tenga identificador;
- la fuente esté registrada;
- las versiones sean distinguibles;
- las decisiones sean trazables;
- los créditos sean correctos;
- las validaciones no se exageren;
- las tareas pendientes se encuentren en el backlog oficial.

## 8.6 Revisión por lotes

Cuando el volumen lo permita, las tareas repetitivas deben procesarse por lotes.

La automatización es adecuada para:

- conversiones;
- normalización técnica;
- metadatos;
- comparaciones;
- detección preliminar de duplicados;
- reportes de cobertura.

La revisión auditiva, lingüística o cultural no debe eliminarse por el hecho de automatizar el procesamiento.

---

# 9. Evolución metodológica

## 9.1 Primera formulación

El proyecto se formuló inicialmente como una herramienta digital basada en:

- escucha reiterada;
- materiales graduados;
- transcripción;
- equivalencia en español;
- tarjetas de repetición espaciada;
- acceso web.

Este núcleo permanece vigente, pero la metodología se volvió más colaborativa y situada.

## 9.2 Abandono de la traducción lineal

El flujo inicial podía resumirse como:

> español escrito → traducción → grabación

Este flujo fue reemplazado por:

> situación real → funciones comunicativas → microescena → elicitación con hablantes → organización pedagógica → producción

El cambio respondió a la artificialidad, las repeticiones y la baja productividad detectadas en los primeros intentos de ampliación del corpus.

## 9.3 Abandono de listas temáticas como arquitectura principal

Los bloques temáticos fueron útiles para iniciar el trabajo, pero resultaron insuficientes para organizar niveles posteriores.

Se mantienen como herramientas auxiliares de cobertura, no como estructura metodológica central.

## 9.4 Separación de responsabilidades

La metodología se refinó al distinguir:

- diseño pedagógico;
- autoridad lingüística;
- autoridad comunitaria;
- revisión técnica;
- control documental;
- generación automática;
- control externo de cobertura.

Esta separación evita pedirle a una sola persona o herramienta que resuelva decisiones para las que no tiene autoridad o capacidad.

## 9.5 Evolución del generador de borradores

La generación automática del corpus en español pasó por varias etapas:

1. generación bilingüe y salida estructurada;
2. eliminación de la producción automática en Didxazá;
3. eliminación de formatos demasiado pesados;
4. prioridad de naturalidad y productividad de patrones;
5. separación entre motor, datos y tarea;
6. traslado del historial y el balance a un controlador externo.

La versión vigente del enfoque automático genera únicamente borradores en español. Esos borradores no entran al corpus sin intervención de hablantes.

## 9.6 Método de mejora

La metodología se modifica mediante:

1. identificación de un problema real;
2. registro de evidencia;
3. análisis de causa;
4. formulación de un ajuste;
5. prueba;
6. revisión;
7. adopción o rechazo;
8. documentación de la decisión.

No se crean reglas nuevas únicamente por anticipar problemas hipotéticos.

---

# 10. Alcances y limitaciones

## 10.1 Alcances

La metodología permite:

- construir materiales auditivos culturalmente situados;
- integrar hablantes en decisiones sustantivas;
- distinguir roles y autoridades;
- organizar una progresión comunicativa;
- conservar trazabilidad;
- revisar y reemplazar decisiones;
- reutilizar estructuras;
- combinar trabajo de campo, bibliografía y producción técnica;
- escalar tareas repetitivas mediante automatización controlada.

## 10.2 Limitaciones lingüísticas

La coordinación no habla Didxazá y depende de la disponibilidad de hablantes para validar el material.

No todas las formas:

- tienen una única escritura;
- se usan del mismo modo en todas las localidades;
- cuentan con documentación suficiente;
- pueden resolverse mediante una sola consulta.

## 10.3 Limitaciones comunitarias

La participación de una persona no equivale a consenso comunitario.

La metodología debe evitar:

- generalizar una variante individual;
- presentar una preferencia como norma universal;
- atribuir autoridad colectiva sin respaldo;
- extraer materiales sin acuerdos claros.

## 10.4 Limitaciones pedagógicas

La metodología todavía no cuenta con validación sistemática mediante grupos de aprendices.

Permanecen pendientes:

- pruebas de comprensión;
- medición de retención;
- evaluación de transferencia;
- ajuste empírico de progresión;
- evaluación de la duración de pausas;
- definición completa de métricas de aprendizaje.

## 10.5 Limitaciones operativas

El proceso depende de:

- tiempo de coordinación;
- disponibilidad de colaboradores;
- recursos de grabación y edición;
- control de versiones;
- herramientas de seguimiento;
- financiamiento;
- formalización de acuerdos.

La automatización reduce carga operativa, pero no elimina la revisión especializada.

## 10.6 Limitaciones del material generado automáticamente

Los modelos pueden producir:

- repeticiones;
- diálogos mecánicos;
- complejidad inadecuada;
- falsa naturalidad;
- patrones desequilibrados;
- inconsistencias entre lotes.

Por ello, sus resultados son borradores sometidos a selección, revisión y validación.

---

# 11. Estado metodológico vigente

Se consideran vigentes:

- enfoque auditivo;
- comprensión progresiva;
- producción integrada;
- situaciones comunicativas como punto de partida;
- funciones comunicativas como unidad funcional;
- colaboración con hablantes;
- separación entre diseño pedagógico y validación lingüística;
- gramática como referencia y control;
- reutilización productiva;
- trazabilidad documental;
- validación según autoridad pertinente;
- automatización limitada a tareas apropiadas;
- revisión de decisiones a partir de evidencia.

Permanecen pendientes de consolidación o prueba:

- protocolo formal de sesiones con hablantes;
- sistema completo de evaluación con aprendices;
- procedimiento uniforme de consentimiento, reconocimiento y uso;
- controlador externo de cobertura;
- validación a escala del generador vigente;
- formalización del criterio exacto de secuenciación entre funciones;
- pruebas de transferencia y retención.
